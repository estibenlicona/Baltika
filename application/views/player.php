    ...
    <?php foreach ($player as $key => $p): ?>
      <div class="row">
        ...
        <div class="col-1 p-2 text-center">
          <a class="state-select nav-item btn btn-dark editar btn-sm" href="<?=base_url('users/edit/'.$p->id)?>" ><i class="opciones-nav icono-btn material-icons md-18 light600">edit</i></a>
        </div>
        <div class="col-1 p-2 text-center">
          <?php  if ($p->estado == 1): ?>
          <a href="<?=base_url('player/status/'.$p->id.'/0/one') ?>" class="btn btn-primary btn-sm">
            <i class="opciones-nav icono-btn material-icons md-18 light600">check</i>
          </a>
          <?php endif; ?>
          <?php if ($p->estado == 0): ?>
          <a href="<?=base_url('player/status/'.$p->id.'/1/one') ?>" class="btn btn-danger btn-sm">
            <i class="opciones-nav icono-btn material-icons md-18 light600">check_box_outline_blank</i>
          </a>
          <?php endif; ?>
        </div>
        <div class="col-1 p-2 text-center">
          <a class="state-select nav-item btn btn-danger btn-sm"  href="<?=base_url('player/status/'.$p->id.'/2/one')?>"><i class="opciones-nav icono-btn material-icons md-18 light600">clear</i></a>
        </div>
      </div>
    <?php endforeach; ?>
  <?php endif; ?>
