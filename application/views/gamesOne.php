<body>
  ...
  <div class="container-fluid">
    <div class="row">
      <div class="col-2 p-2 list-head text-center">
        Date Game
      </div>
      <div class="col-3 p-2 list-head text-left">Home Team</div>
      <div class="col-1 p-2 list-head text-center">Score Home</div>
      <div class="col-1 p-2 list-head text-center">Score Away</div>
      <div class="col-3 p-2 list-head text-right">Away Team</div>
      <div class="col-1 p-2 list-head text-right">Played</div>
      <div class="col-1 p-2 list-head text-left">Saved</div>
    </div>
    <?php foreach ($games as $key => $g): ?>
    <form action="<?=base_url('games/saved/'.$g->id_home.'/'.$g->id_away.'/'.$g->league.'/'.$team)?>" method="post">
    <div class="row">
      ...
      <div class="col-3 p-2 text-left"><?=$g->th_nombre?></div>
      <div class="col-1 p-2 text-center">
        <input class="form-control btn-sm text-center" type="text" value="<?=$g->g_home?>" name="g_home" placeholder="0">
      </div>
      <div class="col-1 p-2 text-center">
        <input class="form-control btn-sm text-center" type="text" value="<?=$g->g_away?>" name="g_away" placeholder="0">
      </div>
      <div class="col-3 p-2 text-right"><?=$g->tv_nombre?></div>
      <div class="col-1 p-2 text-right">
        <?php  if ($g->played == 1): ?>
        <a href="<?=base_url('games/playedOne/'.$g->id_home.'/'.$g->id_away.'/'.$g->league.'/0/'.$team) ?>" class="btn btn-primary btn-sm">
          <i class="opciones-nav icono-btn material-icons md-18 light600">check</i>
        </a>
        <?php endif; ?>
        <?php if ($g->played == 0): ?>
        <a href="<?=base_url('games/playedOne/'.$g->id_home.'/'.$g->id_away.'/'.$g->league.'/1/'.$team) ?>" class="btn btn-danger btn-sm">
